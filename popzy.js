...
function Popzy(options) {
    this.opt = Object.assign({
        closeMothods: ['button', 'overplay', 'escape'],
        destroyOnClose: true,
        cssClass: [],
        footer: false
    }, options);
    ...
    if(!this._template) return console.error(`Khong co ${this.opt.templateId}`);

    const {closeMothods} = this.opt;
    this._allowButtonClose = closeMothods.includes('button');
    this._allowBackdropClose = closeMothods.includes('overplay');
    this._allowEscapeClose = closeMothods.includes('escape');
...
Popzy.prototype._getScrollbarWidth = function() {
    if(this._scrollWidth) return this._scrollWidth;
...
Popzy.prototype.open = function() {
    ...
    // Hieu ung show modal:
    setTimeout(() => this._backdrop.classList.add('popzy--show'), 0)
    ...
    // khi mo modal thi bat xu kien esc de modal
...
Popzy.prototype.close = function(destroy = this.opt.destroyOnClose) {
    ...
    // go bo xu kien keydown khi dong modal
    if(this._allowEscapeClose) {
...
Popzy.prototype._handleEscapeKey = function(e) {
    ...
    if(e.code === 'Escape' && lastModal === this) this.close();
}

Popzy.prototype._onTransitionEnd = function(cb) {
...
Popzy.prototype.addFooterButton = function(title, cssClass, cb) {
    const button = this._createButton(title, cssClass, cb)
    this._footerButtons.push(button);
    
    this._renderFooterButtons();
}

Popzy.prototype._createButton = function(title, cssClass, cb) {
