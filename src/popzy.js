...
function Popzy(options = {}) {
    if(!options.content && !options.templateId) {
        console.error('Ban can nhap content hoac templateId');
        return;
    }

    if(options.content && options.templateId) {
        options.templateId = null;
        console.warn('Neu nhap ca 2 content va template Id thi uu tien content, template se bi bo qua');
    }
...
Popzy.prototype.open = function () {
    ...
    if(this.opt.enableScrollLock) {
        ...
        if(this._hasScrollbar(target)) {
            ...
            const targetPdRight = parseInt(getComputedStyle(target).paddingRight);
            target.style.paddingRight = targetPdRight + this._getScrollbarWidth() + "px";
        }
    }
